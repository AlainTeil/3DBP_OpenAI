...
std::vector<Vec3> orientations_for(const Box& box) {
    ...
    if (box.this_side_up) {
        ...
        result.erase(std::unique(result.begin(), result.end(), [](const Vec3& a, const Vec3& b) {
...
bool overlaps(const Placement& a, const Placement& b) {
    ...
    const bool x_overlap = a.position.w < b.position.w + b.orientation.w &&
                           b.position.w < a.position.w + a.orientation.w;
    const bool y_overlap = a.position.l < b.position.l + b.orientation.l &&
                           b.position.l < a.position.l + a.orientation.l;
    const bool z_overlap = a.position.h < b.position.h + b.orientation.h &&
                           b.position.h < a.position.h + a.orientation.h;
