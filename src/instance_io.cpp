...
nlohmann::json to_json(const PackingResult &result) {
    PackingStatus status = result.status;
    if (result.feasible && result.status == PackingStatus::Infeasible) {
        status = PackingStatus::Feasible;
    }

    nlohmann::json j;
    j["status"] = to_string(status);
    j["feasible"] = status == PackingStatus::Feasible;
    j["objective"] = {{"bins_used", result.objective.bins_used}, {"leftover_volume", result.objective.leftover_volume}};
    j["stats"] = {{"boxes_total", result.stats.boxes_total},       {"boxes_placed", result.stats.boxes_placed},
                  {"boxes_unplaced", result.stats.boxes_unplaced}, {"volume_boxes", result.stats.volume_boxes},
                  {"volume_packed", result.stats.volume_packed},   {"volume_bins_used", result.stats.volume_bins_used},
    ...
    j["unplaced_box_ids"] = result.unplaced_box_ids;
    j["validation_errors"] = result.validation_errors;
    j["placements"] = nlohmann::json::array();
    for (const auto &p : result.placements) {
        j["placements"].push_back(
            {{"box_id", p.box_id},
             {"bin_id", p.bin_id},
...
PackingResult from_json_result(const nlohmann::json &j) {
    ...
    if (j.contains("status")) {
        r.status = packing_status_from_string(j.at("status").get<std::string>());
        r.feasible = r.status == PackingStatus::Feasible;
    } else {
        r.feasible = j.at("feasible").get<bool>();
        r.status = r.feasible ? PackingStatus::Feasible : PackingStatus::Infeasible;
    }
    ...
    if (j.contains("stats")) {
        ...
        r.stats.boxes_unplaced = s.value("boxes_unplaced", 0);
    ...
    if (j.contains("unplaced_box_ids")) {
        r.unplaced_box_ids = j.at("unplaced_box_ids").get<std::vector<std::string>>();
    }
    if (j.contains("validation_errors")) {
        r.validation_errors = j.at("validation_errors").get<std::vector<std::string>>();
    }
    return r;
}
